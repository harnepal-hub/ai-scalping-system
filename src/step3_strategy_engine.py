...
ATR-based SL/TP (5-min ATR):
  ...
  • TP1 = Entry ± 1.0 × ATR  (partial target — half position)
  • TP2 = Entry ± 1.67 × SL distance  (full position exits here; RR = 1.67)
...
def generate_trade_setup(stock: str,
    ...
    -------
    dict
        Keys: stock, direction, session, entry, sl, tp1, tp, tp1_pct,
              qty, max_loss, rr_ratio, sl_pct, tp_pct.
        ``tp`` is the final take-profit (TP2) at ``RR_TARGET`` × sl_dist.
        ``tp1`` is an intermediate partial target at ``TP1_ATR_MULTIPLIER`` × ATR.
    """
    ...
    # ── TP1: partial target at TP1_ATR_MULTIPLIER × ATR ─────────────────────
    tp1_dist = TP1_ATR_MULTIPLIER * atr

    # ── TP2 (final TP): RR_TARGET × clamped SL distance ─────────────────────
    tp2_dist = RR_TARGET * sl_dist

    if direction == "LONG":
        sl  = price - sl_dist
        tp1 = price + tp1_dist
        tp  = price + tp2_dist          # tp == tp2, full position exits here
    else:
        sl  = price + sl_dist
        tp1 = price - tp1_dist
        tp  = price - tp2_dist
    ...
    # ── Risk-Reward (equals RR_TARGET by construction) ────────────────────────
    rr_ratio = RR_TARGET

    # ── Percentage distances ─────────────────────────────────────────────────
    sl_pct   = round(sl_dist   / price * 100, 2)
    tp1_pct  = round(tp1_dist  / price * 100, 2)
    tp_pct   = round(tp2_dist  / price * 100, 2)   # tp_pct == tp2_pct
    ...
    setup = {
        ...
        "sl": round(sl, 2),
        "tp1": round(tp1, 2),
        "tp": round(tp, 2),             # tp == tp2 (final take-profit)
        "qty": qty,
        "max_loss": max_loss,
        "rr_ratio": rr_ratio,
        "sl_pct": sl_pct,
    ...
    logger.info(
        ...
        f"Entry={price:.2f} SL={sl:.2f} TP1={tp1:.2f} TP2={tp:.2f} Qty={qty}"
    )
    return setup
...
def display_trade_card(trade_setup: dict) -> None:
    ...
    tp1 = float(trade_setup.get("tp1", trade_setup.get("tp", 0)))
    ...
    print(_row("Take Profit 2",  f"Rs {tp:>10,.2f}  ({tp_sign}{tp_pct:.2f}%)"))
